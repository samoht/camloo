...
fun print_num n = if n = 0 then output(std_out, makestring n)
                           else output(std_out, makestring n)
fun print_newline () = print_string "\n";
fun message s = (print_string s; print_newline ());
...
fun Group_precedence op1 op2 =
  ...
  if (op1 = "I") orelse (op2 = "U") then Greater else NotGE;
...
fun Geom_rank "U" = 0
  ...
  | Geom_rank "C" = 4
  | Geom_rank "A" = 5
  | Geom_rank _ = failwith "Geom_rank"
;

fun Geom_precedence op1 op2 =
  let val r1 = Geom_rank op1;
      val r2 = Geom_rank op2
  in
...
fun doit0() = kb_complete (gt_ord Group_order) [] Group_rules

fun doit() = kb_complete (gt_ord Geom_order) [] Geom_rules
