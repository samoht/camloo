...
let get name =
  ...
  | [] ->
      let entry = {name = name; props = []} in
        lemmas := entry :: !lemmas;
        entry
  in get_rec !lemmas in

let add_lemma (Prop(_, [(Prop(headl,_) as left); right])) =
  headl.props <- (left, right) :: headl.props in
...
and rewrite_with_lemmas = fun
  ...
  | term ((t1,t2)::rest) ->
      ...
      with Unify ->
        rewrite_with_lemmas term rest in

let rec cterm_to_term = function
    CVar v -> Var v
  | CProp(p, l) -> Prop(get p, map cterm_to_term l) in

let add t = add_lemma (cterm_to_term t) in
