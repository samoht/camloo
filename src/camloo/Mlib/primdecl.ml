...
let find_primitive arity name =
  try
    ValuePrim(Regprim, arity, assoc name primitive_names)
  with Not_found ->
    ValuePrim(Regprim, arity, Pccall(name, arity))
;;

let find_scheme_primitive arity name module_name file_name =
  ValuePrim(Schemeprim(module_name, file_name), 
            arity, 
            Pccall(name, arity))
;;
