...
let rec size_of_expr (Expr(e,loc)) =
  match e with
    Ztuple el ->
      do_list check_letrec_expr el; Dummy_tuple( list_length el )
  | Zconstruct1(cstr, expr) ->
      check_letrec_expr expr;
      begin match cstr.info.cs_kind with
        Constr_superfluous n -> Dummy_construct1( cstr, n ) 
                         | _ -> Dummy_construct1( cstr, 1 )
      end
  | Zfunction _ ->
      Dummy_function
  | Zconstraint(e,_) ->
      size_of_expr e
  | Zvector el ->
      do_list check_letrec_expr el; Dummy_vector( list_length el )
  | Zrecord lbl_expr_list ->
      do_list (fun (lbl,expr) -> check_letrec_expr expr) lbl_expr_list;
      Dummy_record( list_length lbl_expr_list )
...
let rec translate_expr env =
  ...
  | Ztuple(args) ->
      let tr_args =
        map transl args in
      begin try
        Lconst(SCblock(ConstrRegular({qual="builtin"; id="tuple"},0,1), map extract_constant tr_args))
      with Not_constant ->
        Lprim(Pmakeblock(ConstrRegular({qual="builtin"; id="tuple"},0,1)), tr_args)
      end
  | Zconstruct0(c) ->
  ...
  | Zconstraint(E, _) ->
      transl E
  | Zvector [] ->
      Lconst(SCblock(ConstrRegular({qual="builtin"; id="vector"},0,0), []))
  | Zvector args ->
      Lprim(Pmakeblock (ConstrRegular({qual="builtin"; id="vector"},0,0)), 
                                     map transl args)
  | Zassign(id, E) ->
      ...
      let v = make_vect (list_length lbl_expr_list) Lstaticfail in
        ...
        begin try
          if for_all
               ...
               lbl_expr_list
          then Lconst(SCblock(ConstrRegular({qual="builtin"; id="record"},0,0),
                              map_vect_list extract_constant v))
          else raise Not_constant
        with Not_constant ->
          Lprim(Pmakeblock(ConstrRegular({qual="builtin"; id="record"},0,0)),
                          list_of_vect v)
        end
