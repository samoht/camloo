...
let rec compile_expr staticfail =
  ...
  | Lfor(start, stop, up_flag, body) ->
        ...
        and lbl_loop = new_label() in
          compexp start (
            Kmakeblock(ConstrRegular({qual="ref"; id="ref"},0,1), 1) :: Klet ::
            compexp stop (
  ...
  | Lswitch(size, arg, cases) ->
        let casel =
         map (function (c,act) -> (c.info.cs_tag, act)) cases in
        begin match size, arg, casel with
         (1, arg, [ConstrRegular(_,_,_), exp]) ->
        compexp exp C
        (* En supposant que l'argument est toujours du code pur !!!
           (vrai quand c'est le pattern-matcher qui genere le Switch).     *)
       | (2, arg, [ConstrRegular(_,0,_), exp0]) ->
         compexp arg (Kbranchif staticfail :: compexp exp0 C)
       | (2, arg, [ConstrRegular(_,1,_), exp1]) ->
         compexp arg (Kbranchifnot staticfail :: compexp exp1 C)
       | (2, arg, [ConstrRegular(_,0,_), exp0; ConstrRegular(_,1,_), exp1]) ->
         comp_test2 arg exp1 exp0 C
       | (2, arg, [ConstrRegular(_,1,_), exp1; ConstrRegular(_,0,_), exp0]) ->
         comp_test2 arg exp1 exp0 C
       | (size, arg, cases) ->
         let C1 =
