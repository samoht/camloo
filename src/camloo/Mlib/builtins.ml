...
;;
let builtin_constr n = {qual="builtin"; id=n}
;;
...
let constr_void =
  builtin "()"
    { cs_res = {typ_desc=Tconstr(constr_type_unit,[]); typ_level=notgeneric};
      cs_arg = type_unit;
      cs_tag = ConstrRegular(builtin_constr "()",0,1);
      cs_mut = Notmutable;
...
let constr_nil =
  ...
  builtin "[]"
    { cs_res = {typ_desc=Tconstr(constr_type_list, [arg]); typ_level=generic};
      cs_arg = type_unit;
      cs_tag = ConstrRegular(builtin_constr "[]",0,2);
      cs_mut = Notmutable;
      cs_kind= Constr_constant }
...
and constr_cons =
  ...
  builtin "::"
    { cs_res = arg2;
      ...
      cs_tag = ConstrRegular(builtin_constr "::",1,2);
      cs_mut = Notmutable;
...
let constr_false =
  builtin "false"
    { cs_res = {typ_desc=Tconstr(constr_type_bool,[]); typ_level=notgeneric};
      cs_arg = type_unit;
      cs_tag = ConstrRegular(builtin_constr "false",0,2);
      cs_mut = Notmutable;
      cs_kind= Constr_constant }
...
and constr_true =
  builtin "true"
    { cs_res = {typ_desc=Tconstr(constr_type_bool,[]); typ_level=notgeneric};
      cs_arg = type_unit;
      cs_tag = ConstrRegular(builtin_constr "true",1,2);
      cs_mut = Notmutable;
      cs_kind= Constr_constant }
;;
