...
(* !!! WARNING !!! WARNING !!! WARNING !!! WARNING !!! WARNING !!!
   ---------------------------------------------------------------
   il est obligatoire de changer le type `stream' dans ce fichier et
   pas forcement dans le compilo camlc. C'est un gros hack qui marche.
   Ici, on doit avoir `SBempty of int' pour l'histoire du update. *)
type 'a stream =
    Sempty
  | Scons of 'a * 'a stream
  ...
  | SBempty of int (* Attention, c'est vraiment un gros hack, SBempty doit    
                      etre le 6ieme champs de la structure (slot 5) *)
;;

let rec stream_peek = function
    Sempty -> raise Parse_failure
  | SBempty _ -> raise Parse_failure
  | Scons(x,_) -> x
  | Sapp(s1, s2) as s ->
      begin try
        stream_peek s1
