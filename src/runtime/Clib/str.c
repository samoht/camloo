...
// value len;
// {
//  return alloc_string(Long_of_val(len));
//}
...
//       len--, p1++, p2++)
//    if (*p1 != *p2)
//      return (*p1 < *p2 ? Val_of_long(-1) : Val_of_long(1));
//  if (len1 == len2)
//    return Val_of_long(0);
//  else if (len1 < len2)
//    return Val_of_long(-2);
//  else
//    return Val_of_long(2);
//}
...
//{
//  bcopy(&Byte(s1, Long_of_val(offset1)),
//        &Byte(s2, Long_of_val(offset2)),
//        Int_val(len));
//  return Atom(0);
//}
...
//  c = CCHAR(init);
//  for(p = &Byte(s, Long_of_val(offset)), n = Long_of_val(len);
//      n > 0; n--, p++)
//    *p = c;
//  return Atom(0);
//}
     ...
     value chr;
{
  ...
  c = CCHAR(chr);
  return Val_of_bool(printable_chars[c >> 3] & (1 << (c & 7)));
}
